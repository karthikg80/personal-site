...
import { executePublish } from '../../../lib/publishing/publish-flow';
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    return json({ error: 'The Publish request could not be read.' }, 400);
  }

  const result = await executePublish(payload);
  if (!result.ok) {
    return json({ error: result.error }, result.status);
  }

  console.info('drafting publish', { slug: result.slug, objectId: result.objectId });
