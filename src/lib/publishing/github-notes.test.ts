...
describe('GitHub notes Contents client', () => {
  ...
  it('decodes a note file blob', async () => {
    const fetchMock = vi.fn(async () => jsonResponse(200, filePayload(markdown, 'abc123')));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('does not scan the notes directory when recovering with a slug', async () => {
    ...
    expect(fetchMock.mock.calls.every(([url]) => String(url).includes(`${slug}.md`))).toBe(true);
  });
  ...
  it('scans the notes directory only when slug is missing', async () => {
    ...
    expect(fetchMock.mock.calls.some(([url]) => String(url).endsWith('/contents/src/content/notes?ref=main'))).toBe(
      true
    );
  });
